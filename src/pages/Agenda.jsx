...
function ModalEvento({ sesion, instructoras, espacios, onClose, onSaved, onCancelSesion }) {
  ...
  const instrNombre = instructoras.find(i => i.id === sesion.instructora_id)?.nombre ?? sesion.instructoras?.nombre ?? '—'
  ...
  async function guardar() {
    ...
    if (sesion.es_recurrente && sesion.plantilla_id) {
      // Editar toda la serie: actualizar plantilla + todas las sesiones futuras (y pasadas no canceladas)
      const payload = {
      ...
      const [{ error: e1 }, { error: e2 }] = await Promise.all([
        ...
        supabase.from('sesiones').update(payload)
          .eq('plantilla_id', sesion.plantilla_id)
          .eq('cancelada', false),
      ])
      if (e1 || e2) { setError('Error al guardar: ' + (e1?.message ?? e2?.message)); setSaving(false); return }
    } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
        ...
        {/* Header con color de la clase */}
        <div className="h-1.5" style={{ background: `linear-gradient(90deg, ${color}, ${color}99)` }} />
        <div className="px-6 pt-5 pb-4 flex items-start justify-between">
          <div>
            <p className="label-caps mb-1">{sesion.es_recurrente ? 'Clase recurrente' : 'Clase única'}</p>
            <h3 className="font-display" style={{ fontSize:28, color:'#111827', letterSpacing:'-0.02em', lineHeight:1 }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} style={{ color:'#9CA3AF' }} className="p-1 hover:text-gray-600 transition-colors">
              <X size={18} />
            </button>
          </div>
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { icon: Users,  label:'Ocupación', val:`${sesion.reservas_count}/${sesion.capacidad_max} (${occ}%)` },
              ...
              <div>
                <div className="flex justify-between mb-1.5">
                  <p className="label-caps">Ocupación</p>
                  <p className="label-caps" style={{ color: occ>=90 ? '#B34C60' : occ>=70 ? '#B07D1A' : '#9CA3AF' }}>
                    {occ}%
                  </p>
                </div>
                <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor:'#F0F4F1' }}>
                  <div className="h-full rounded-full transition-all"
                    style={{ width:`${occ}%`, backgroundColor: occ>=90 ? '#B34C60' : occ>=70 ? '#B07D1A' : '#4F7C44' }} />
                </div>
              </div>
              ...
              {!confirmarCancel ? (
                ...
                <div className="rounded-xl p-4" style={{ backgroundColor:'#FEF2F2', border:'1px solid #FECACA' }}>
                  <p className="text-sm font-medium mb-3" style={{ color:'#991B1B' }}>
                    ¿Confirmar cancelación de esta clase?
                  </p>
            ...
            <>
              {sesion.es_recurrente && (
                <div className="flex items-start gap-2 rounded-lg px-3 py-2.5"
                  ...
                  <p className="text-xs" style={{ color:'#9A3412' }}>
                    Los cambios se aplicarán a <strong>toda la serie</strong> — pasadas y futuras.
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" className="input" value={edit.hora_inicio}
                    onChange={e => setEdit(f => ({ ...f, hora_inicio:e.target.value }))} />
                </div>
                <div>
                  <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>
                    Hora fin *
                  </label>
                  <input type="time" className="input" value={edit.hora_fin}
                    onChange={e => setEdit(f => ({ ...f, hora_fin:e.target.value }))} />
                </div>
              </div>
  ...
  function handleTipoChange(tipo_id) {
    ...
    if (tipo) {
      ...
      setForm(f => ({ ...f, tipo_clase_id: tipo_id,
        hora_fin: `${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}` }))
    } else setForm(f => ({ ...f, tipo_clase_id: tipo_id }))
  }
  ...
  async function crearSesion() {
    ...
    if (form.es_recurrente) {
      if (form.dias_semana.length === 0) {
        setError('Selecciona al menos un día de la semana.'); setSaving(false); return
      }
      ...
      if (fechas.length === 0) { setError('Sin fechas válidas en el rango.'); setSaving(false); return }
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} style={{ color:'#9CA3AF' }} className="hover:text-gray-600 transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Tipo de clase *</label>
            <select className="input" value={form.tipo_clase_id} onChange={e => handleTipoChange(e.target.value)}>
              <option value="">Selecciona una clase</option>
              {tiposClase.map(t => <option key={t.id} value={t.id}>{t.nombre} ({t.duracion_min} min)</option>)}
            </select>
          </div>
          <div>
            <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Instructora *</label>
            <select className="input" value={form.instructora_id}
              onChange={e => setForm(f => ({ ...f, instructora_id:e.target.value }))}>
              <option value="">Selecciona instructora</option>
              {instructoras.map(i => <option key={i.id} value={i.id}>{i.nombre} {i.apellidos}</option>)}
            </select>
          </div>
          <div>
            <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Espacio *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Hora inicio</label>
              <input type="time" className="input" value={form.hora_inicio}
                onChange={e => setForm(f => ({ ...f, hora_inicio:e.target.value }))} />
            </div>
            <div>
              <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Hora fin</label>
              <input type="time" className="input" value={form.hora_fin}
                onChange={e => setForm(f => ({ ...f, hora_fin:e.target.value }))} />
            </div>
          </div>
          <div>
            <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Capacidad máxima</label>
          ...
          <div className="rounded-xl p-4" style={{ backgroundColor:'#F5F7F5', border:'1px solid #E2E8E2' }}>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p style={{ fontSize:11, color:'#9CA3AF' }}>Se programa en los días seleccionados</p>
              </div>
            </label>
          </div>
          ...
          {form.es_recurrente ? (<>
            <div>
              <label className="block mb-2" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Días de la semana *</label>
              <div className="flex gap-2">
                {DIAS_SEM.map(d => {
                  ...
                  return (
                    <button key={d.value} type="button" onClick={() => toggleDia(d.value)}
                      ...
                      style={{ backgroundColor: sel ? '#3E6335' : '#F0F4F1', color: sel ? 'white' : '#9CA3AF',
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Fecha inicio *</label>
            ...
            <div>
              <label className="block mb-1.5" style={{ fontSize:12, fontWeight:500, color:'#4B5563' }}>Fecha *</label>
...
export default function Agenda() {
  ...
  const [sesionSel, setSesionSel] = useState(null)   // para modal edición
  ...
  async function loadSesiones() {
    ...
    const { data } = await supabase.from('sesiones')
      .select(`id,fecha,hora_inicio,hora_fin,capacidad_max,reservas_count,
               cancelada,plantilla_id,es_recurrente,instructora_id,espacio_id,tipo_clase_id,
                        ...
                        {slots.map(s => {
                          ...
                          return (
                            <div key={s.id}
                              ...
                              onMouseEnter={e => { e.currentTarget.style.backgroundColor = color+'28'; e.currentTarget.style.transform = 'scale(1.01)' }}
                              onMouseLeave={e => { e.currentTarget.style.backgroundColor = color+'18'; e.currentTarget.style.transform = 'scale(1)' }}>
                              <div className="flex items-center justify-between gap-1">
                                <p className="font-semibold leading-tight flex-1 truncate" style={{ color:'#1C2A22' }}>
